function extractAndSaveEMGFeatures()
    ...
    windowSize = round(fs * 0.3); % Window size for feature extraction (e.g., 0.3 seconds)
        ...
        if ~isempty(axesHandles)
            ...
            if ~isempty(lineHandles)
                ...
                while startIdx + windowSize - 1 <= length(filteredEMG)
                    ...
                    % Check if features have consistent dimensions before storing
                    if size(features, 1) ~= expectedFeatureCount() 
                        fprintf('Warning: Inconsistent feature size at segment starting index: %d\n', startIdx);
                    else
    ...
    save(fullfile(pathname, 'olExtractedFeatures.mat'), 'allFeatures');
    ...
    save(fullfile(pathname, 'olPreparedData.mat'), 'featureList', 'labelList');
    ...
    excelFilePath = fullfile(pathname, 'olPreparedData.xlsx'); 
