function extractAndSaveEMGFeatures()
    ...
    windowSize = round(fs * 0.1); % Window size for feature extraction (e.g., 0.3 seconds)
    ...
    for k = 1:length(filenames)
        ...
        if ~isempty(axesHandles)
            ...
            if ~isempty(lineHandles)
                ...
                fprintf('Length of filtered EMG data: %d\n', length(filteredEMG));
                ...
                while startIdx + windowSize - 1 <= length(filteredEMG)
                    ...
                    if isempty(segment)
                        fprintf('Warning: Empty segment at index: %d\n', startIdx);
                        break; % Exit if segment is empty
                    end
                    ...
                    if isempty(features)
                        fprintf('Warning: No features extracted from segment at index: %d\n', startIdx);
                    else
                        fprintf('Extracted features from segment starting at index: %d\n', startIdx);
                        
                        % Update maximum feature length and pad if necessary
    ...
    for i = 1:length(fieldNames)
        ...
        % Pad with zeros to match maxFeatureLength for each row of features
        for j = 1:size(features, 1)  % Iterate through each row (feature set)
            if size(features(j,:), 2) < maxFeatureLength 
                features(j,end+1:maxFeatureLength) = 0; 
            elseif size(features(j,:), 2) > maxFeatureLength 
                features(j,:) = features(j, 1:maxFeatureLength); 
            end
        end
    ...
    save(fullfile(pathname, 'nExtractedFeatures.mat'), 'allFeatures');
    ...
    save(fullfile(pathname, 'nPreparedData.mat'), 'featureList', 'labelList');
    ...
    excelFilePath = fullfile(pathname, 'nPreparedData.xlsx'); 
...
function expectedCount= expectedFeatureCount()
   expectedCount=4+length(0:0.01:10);% Example based on time domain + wavelet transform + STFT.
end
