...
% Average the filtered EMG channels to get a single signal
averaged_signal = mean(filtered_emg_channels, 2); % Average across rows (time points)
...
% Plotting the averaged signal in a new figure
figure;
plot(time_sec, averaged_signal, 'b'); % Averaged signal in blue
title('Averaged Filtered EMG Signal');
xlabel('Time (s)');
ylabel('Amplitude (V)');
grid on;
xlim([0 max(time_sec)]);
